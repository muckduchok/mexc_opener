...
```bash
npm install
cp .env.example .env   # задайте DATABASE_URL и прочие дефолты
```

`config.json` **создаётся автоматически** при первом запуске, если его нет (пустой скелет
`{ "accounts": {}, "groups": [] }`). В режиме с `DATABASE_URL` аккаунты и группы тянутся из MongoDB,
поэтому заполнять его вручную не нужно. Статический режим (без БД) — впишите аккаунты/группы
сами (см. `config.example.json`).

## Конфигурация

`config.json` — аккаунты и хедж‑группы (оба блока могут быть пустыми в режиме с MongoDB):
